...
version <- "allModelsVersion3.2.sens.A_roof.det"
...
parameters <- parameters[which(names(parameters) != "A_roof")]

# run simulations for all variations of A_roof
MC_result <- c()
startTime <- Sys.time()
for (i in 1:length(variationsA_roof)) {
  print(paste0("Processing variation ", i, " of ", length(variationsA_roof), " for A_roof"))

  tmp <- MC_process(parameters = c(parameters, A_roof = variationsA_roof[i]), n_mc = n_mc, yini = yini)
  MC_result <- cbind(MC_result, c(A_roof = variationsA_roof[i], tmp))
}
MC_result <- as.data.frame(t(MC_result))

{
  plot.h <- ggplot(MC_result) +
    geom_point(aes(x = A_roof, y = h)) +
    scale_color_ipsum() +
    scale_fill_ipsum() +
  ...
  plot.bottlesPerDay <- ggplot(MC_result) +
    geom_point(aes(x = A_roof, y = bottlesPerDay)) +
    scale_color_ipsum() +
    scale_fill_ipsum() +
    theme_ipsum_ps(grid="XY", axis="xy") + theme(legend.position="none")
  plot.bottlesPerDay
  ...
  plot.T_water <- ggplot(MC_result) +
    geom_point(aes(x = A_roof, y = T_water)) +
    scale_color_ipsum() +
    scale_fill_ipsum() +
    theme_ipsum_ps(grid="XY", axis="xy") + theme(legend.position="none")
  plot.T_water
